...
use App\Http\Requests\Member\OfficeExpenses\StoreOfficeExpense;
//use App\Notifications\NewExpenseAdmin;
//use App\Notifications\NewOfficeExpenseAdmin;
//use App\Project;
//use App\ExpensesCategory;
use App\OfficeExpenseCategory;
use App\Role;
use App\User;
use Carbon\Carbon;
 ...
 */
class MemberOfficeExpenseController extends MemberBaseController
{

    public function __construct()
    { 
        parent::__construct();
        $this->pageTitle = 'app.menu.officeExpenses';
    ...
    public function index()
    {
        ...
        return view('member.office-expenses.index', $this->data);
    }

    public function data(Request $request)
    {
        $payments = OfficeExpense::with(['user']);
        if($request->startDate !== null && $request->startDate != 'null' && $request->startDate != ''){
            $startDate = Carbon::createFromFormat($this->global->date_format, $request->startDate)->format('Y-m-d');
            $payments = $payments->where(DB::raw('DATE(office_expenses.`purchase_date`)'), '>=', $startDate);
        }

        if($request->endDate !== null && $request->endDate != 'null' && $request->endDate != ''){
            $endDate = Carbon::createFromFormat($this->global->date_format, $request->endDate)->format('Y-m-d');
            $payments = $payments->where(DB::raw('DATE(office_expenses.`purchase_date`)'), '<=', $endDate);
        }

        if($request->status != 'all' && !is_null($request->status)){
            $payments = $payments->where('office_expenses.status', '=', $request->status);
        }
        if($request->status != 'all' && !is_null($request->status)){
            $payments = $payments->where('office_expenses.status', '=', $request->status);
        }
        ...
        //if(!$this->user->cans('view_expenses')){
            $payments = $payments->where('office_expenses.user_id', '=', $this->user->id);
        //}
        ...
        $dataTable = DataTables::of($payments)
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->status == 'pending') {
                    $html .= '<a href="' . route('member.office-expenses.edit', $row->id) . '" data-toggle="tooltip" data-original-title="Edit" class="btn btn-info btn-circle"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;';
                }
    ...
    public function create()
    {
        ...
        $this->categories = OfficeExpenseCategory::join('expenses_category_roles', 'expenses_category_roles.expenses_category_id', 'expenses_category.id')
        ...
        return view('member.office-expenses.create', $this->data);
    }

    public function store(StoreOfficeExpense $request)
    {
        $expense = new OfficeExpense();
        ...
        return Reply::redirect(route('member.office-expenses.index'), __('messages.officeExpenseSuccess'));
    }
    ...
    public function edit($id)
    {
        ...
        //     abort(403);
        // }
        $this->expense = OfficeExpense::findOrFail($id)->withCustomFields();
        ...
        return view('member.office-expenses.edit', $this->data);
    }

    public function update(StoreOfficeExpense $request, $id)
    {
        // if(!$this->user->cans('edit_expenses')){
        //     abort(403);
        // }
        ...
        return Reply::redirect(route('member.office-expenses.index'), __('messages.officeExpenseUpdateSuccess'));
    }
    ...
    public function destroy($id)
    {
        ...
        //     abort(403);
        // }
        OfficeExpense::destroy($id);
        return Reply::success(__('messages.officeExpenseDeleted'));
    }


    public function mem_freeEmployees(Type $var = null)
    {
       

    //echo '<pre>';print_r($notAssignedProject);die;
        return view('admin.mem_free_employees', $this->data);
    }

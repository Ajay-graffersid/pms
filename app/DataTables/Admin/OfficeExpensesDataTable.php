...
class OfficeExpensesDataTable extends BaseDataTable
{
     ...
     */
    public function query(OfficeExpense $model)
    {
        $request = $this->request();
        $company = company();
        $model = $model->select('office_expenses.id', 'office_expenses.item_name',  'office_expenses.user_id', 'office_expenses.price', 'users.name', 'office_expenses.purchase_date', 'office_expenses.currency_id', 'currencies.currency_symbol', 'office_expenses.status', 'office_expenses.purchase_from', 'office_expenses.expenses_recurring_id')
            ->join('users', 'users.id', 'office_expenses.user_id')
            ->join('currencies', 'currencies.id', 'office_expenses.currency_id')
           // ->leftjoin('projects', 'projects.id', 'expenses.project_id')
            ->where('office_expenses.company_id', $company->id);
           // ->where('expenses.company_id', 2);

        // if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
        //     $startDate = Carbon::createFromFormat($this->global->date_format, $request->startDate)->toDateString();
        //     $model = $model->where(DB::raw('DATE(office_expenses.`purchase_date`)'), '>=', $startDate);
        // }

        // if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
        //     $endDate = Carbon::createFromFormat($this->global->date_format, $request->endDate)->toDateString();
        //     $model = $model->where(DB::raw('DATE(office_expenses.`purchase_date`)'), '<=', $endDate);
        // }

        // if ($request->status != 'all' && !is_null($request->status)) {
        //     $model = $model->where('office_expenses.status', '=', $request->status);
        // }
        // if ($request->employee != 'all' && !is_null($request->employee)) {
        //     $model = $model->where('office_expenses.user_id', '=', $request->employee);
        // }
        // if (!is_null($request->category_id) && $request->category_id != 'all') {
        //     $model->where('office_expenses.category_id', $request->category_id);
        // }

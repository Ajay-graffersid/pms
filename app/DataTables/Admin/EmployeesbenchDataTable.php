...
class EmployeesbenchDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn(
                'status',
                function ($row) {
                    if ($row->status == 'active' && $row->status2=='active') {
                        return '<label class="label label-success">' . __('app.active'). '</label>';
                    }
                    if ($row->status == 'deactive' && $row->status2 == 'deactive') {
                        return '<label class="label label-success">' . __('app.inactive'). '</label>';
                    }
                    if ($row->status == 'active' && $row->status2=="bench") {
                        return '<label class="label label-success">' . __('app.bench'). '</label>';
                    }
                    else {
                        return '<label class="label label-danger">' . __('app.bench') . '</label>';
                    }
                }
            )
